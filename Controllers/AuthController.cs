...
namespace OntuPhdApi.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> SignOut()
        {
            ...
            var response = new { message = "Signed out" };
            _logger.LogInformation("SignOut successful, response: {Message}", response.message);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> GetCurrentUser()
        {
            ...
            _logger.LogInformation("Extracted user ID from token: {UserId}", userIdString);

            if (string.IsNullOrEmpty(userIdString) || !int.TryParse(userIdString, out var userId))
            {
                _logger.LogError("GetCurrentUser failed: Invalid user ID in token");
